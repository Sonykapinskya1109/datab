...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void DeleeOne(String name){
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(MY_TABLE, COLUMN_NAME+"= ?", new String[]{name});
        db.close();
    }

    public void UpfateOne(String name){
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues cv = new ContentValues();
        cv.put(COLUMN_NAME, name);
        cv.put(COLUMN_SURNAME, "Snith");
    ...
    public long insert1000(){
        ...
        String sql_inequery = "INSERT INTO " +MY_TABLE+" VALUES(?,?,?);";
        SQLiteStatement statement = db.compileStatement(sql_inequery);

        try {
            for(int i = 0; i!= 1000; i++){
                statement.clearBindings();

                statement.bindString(1, "Vasiiy");
